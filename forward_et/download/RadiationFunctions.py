...
def calculateRLDownInstIDsoandJackson(LST, Tmax, Tmin, latitude, longitude, year, step):
    ...
    # Converts the year and step from the netcdf file to gregorian calendar so
    # it can be use in the function from noaa.
...
def splitNetRadiationCanopySoil(Rn, LAI):
    ...
    # Rns is calculated as it follows in García et al (2013) which Fisher says
    # is based in Beer (1852)
    RnS = Rn**(-kRn * LAI)  # (Wm-2)
