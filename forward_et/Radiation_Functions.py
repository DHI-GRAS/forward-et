...
def CalShortWaveIncomingRadiation(SunHours,Latitude,Doy):
    ...
    Ra=(((24*60)/np.pi)*0.0820)*dr*((ws*np.sin(latRad)*np.sin(delta))+(np.cos(latRad)*np.cos(delta)*np.sin(ws)))# Mjm2day-1
...
def CalLongWave_Incoming_Parton_Logan(LST,ObsTime,N,AirMax,AirMin):
    import numpy as np
    import os
    c1=-0.17; #c= lag of the minimum temperature from the time of sunrise,(parameter following table 1, p.210)
    d1=1.86;  #d= d refers to a in Parton and Logan, (parameter following table 1, p.210)
    BB=12-(N/2)+c1
