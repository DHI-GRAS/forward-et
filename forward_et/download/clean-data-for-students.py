...
for date in date_list:
    print date
    try:
        Test, Test2 = GetMODISdata.DownloadAllData(
            tiles, date, date_list_8days, Test, Test2, MainFolder)
        ...
        Dictionary = Prepare_input_data.FillReflectanceVariables(
            MainFolder, Dictionary, JulianDay, year2process, reference_metadata)
        NDVI16 = Prepare_input_data.FillNDVI_16_Days(
            MainFolder, Dictionary, JulDay16, year2process, reference_metadata)
        Dictionary = Prepare_input_data.FillLSTVariables(
            MainFolder, Dictionary, JulianDay, year2process, reference_metadata)
        Dictionary = Prepare_input_data.FillAlbedoInputs(
            MainFolder, Dictionary, JulDay8, year2process, reference_metadata)
        Dictionary = Prepare_input_data.FillAPAR_and_LAI_Inputs(
            MainFolder, Dictionary, JulDay8, year2process, reference_metadata)
        Dictionary = Prepare_input_data.fillECMWF_Variables(
            MainFolder, Dictionary, JulianDay, year2process)
