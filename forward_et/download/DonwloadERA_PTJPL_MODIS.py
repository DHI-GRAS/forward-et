...
def multiplyBy100(fileName, bandNum):
    ...
    dsOut = driver.Create(outFileName, ds.RasterXSize, ds.RasterYSize, 1, gdal.GDT_Int16)  
...
def gdal2GeoTiff_ECMWF_WGS84(filename):
    ...
    for i in range(1,bandsNum+1,8):
        ...
        forecastSeconds = int(re.findall('\d+',band.GetMetadata()['GRIB_FORECAST_SECONDS'])[0])
        UTCtime_delta = int(re.findall('\d+',band.GetMetadata()['GRIB_REF_TIME'])[0]) + forecastSeconds        
...
#                        str((d + timedelta(seconds=UTCtime_delta)).hour).zfill(2) + '_ECMWF.tif')
        ...
        if productName == '2T':
            ...
            gdalWarpCmd = C_gdalwarp+' -wm 2000 -s_srs "'+srcProj+'" -t_srs "'+dstProj+'" -tr '+dstTr+' -te '+dstTe+' -r cubicspline -ot Int16 -co COMPRESS=DEFLATE -co PREDICTOR=2 -co ZLEVEL=9 -overwrite '+tempFile+' '+tiff_filename
        if productName == 'SSRD':
            # Test to correct
            tempFile = dailySSRD(filename, i)    
            gdalWarpCmd = C_gdalwarp+' -wm 2000 -s_srs "'+srcProj+'" -t_srs "'+dstProj+'" -tr '+dstTr+' -te '+dstTe+' -r near -ot UInt32 -co COMPRESS=DEFLATE -co PREDICTOR=2 -co ZLEVEL=9 -overwrite '+tempFile+' '+tiff_filename
